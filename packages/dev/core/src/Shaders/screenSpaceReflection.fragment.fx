...
ReflectionInfo getReflectionInfo2DRayMarching(vec3 dirVS, vec3 hitCoordVS, vec2 texSize){
    ...
    float minTol; // use to compute maxTol offset. Then acts as filter (2 jobs, 1 variable)
    ...
    for (int i = 0; i < int(delta); i++) {
        ...
        minTol = thickness * 0.01; // minTol useless now, we will use it to apply a filter for depth evaluation
